...
mod tests {
    ...
    #[test]
    fn protocol_storage_preserves_access_warmth_and_rolls_back() {
        ...
        for initially_warm in [false, true] {
            ...
            db.insert_account_storage(address, key, U256::from(3u64))
                .unwrap();
            let mut context: Context<BlockEnv, TxEnv, CfgEnv, CacheDB<EmptyDB>> =
                Context::new(db, SpecId::BERLIN);
            ...
            let checkpoint = journal.frame_transaction_checkpoint();
            assert_eq!(
                journal.frame_transaction_storage(address, key).unwrap(),
                Some(U256::from(3u64))
            );
            assert!(journal
                .set_frame_transaction_storage(address, key, U256::from(4u64))
                .unwrap());
            ...
            assert_eq!(
                journal.load_account(address).unwrap().is_cold,
                !initially_warm
            );
            let slot = journal.sstore(address, key, U256::from(5u64)).unwrap();
            assert_eq!(slot.is_cold, !initially_warm);
            assert_eq!(slot.original_value, U256::from(3u64));
            assert_eq!(slot.present_value, U256::from(4u64));
            journal.frame_transaction_checkpoint_revert(checkpoint);
            assert_eq!(
                journal.frame_transaction_storage(address, key).unwrap(),
                Some(U256::from(3u64))
            );
            ...
            assert_eq!(
                journal.sload(address, key).unwrap().is_cold,
                !initially_warm
            );
        }
    }
